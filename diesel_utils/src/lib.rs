...
impl Timespan for DieselTimespan {
    ...
    fn lower(self) -> DateTime<Utc> {
        match self.0 {
            ...
            Excluded(x) => x,
            Unbounded => panic!(
                "Why the flying fudge is there an unbounded timestamp IN MY GOD DAMN DATABASE!!"
            ),
        }
    }
...
macro_rules! update {
    ($conn:expr, $table_name:ident, $pkey:ident, $changeset:expr) => {
        diesel::update(schema::$table_name::table)
            .filter(schema::$table_name::dsl::$pkey.eq($changeset.$pkey))
            .set($changeset)
            .get_result($conn);
    };
}

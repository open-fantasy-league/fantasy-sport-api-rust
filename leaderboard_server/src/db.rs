...
pub fn get_full_leagues(conn: &PgConn, league_id_filter: Option<Vec<&Uuid>>, leaderboard_id_filter: Option<Vec<&Uuid>>) -> Result<Vec<ApiLeaderboard>, diesel::result::Error>{
    let leaderboards: Vec<Leaderboard> = match (league_id_filter, leaderboard_id_filter){
        (None, None) => {
            schema::leaderboards::table.load(conn)
        },
        (Some(league_ids), None) => {
            schema::leaderboards::table.filter(schema::leaderboards::league_id.eq(any(league_ids))).load(conn)
        },
        (None, Some(leaderboard_ids)) => {
            schema::leaderboards::table.filter(schema::leaderboards::leaderboard_id.eq(any(leaderboard_ids))).load(conn)
        },
        _ => panic!("cant be bothered.")
    }?;
    let players: Vec<Stat> = Stat::belonging_to(&leaderboards).order(schema::stats::points.desc()).load(conn)?;

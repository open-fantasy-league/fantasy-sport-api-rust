...
pub async fn insert_draft_pick(
    method: &str, message_id: Uuid, data: DraftPick, conn: PgConn, ws_conns: &mut WSConnections_, 
    ...
    let pick_ids = conn.build_transaction().run(|| {
        ...
        let league = match leagues.first(){
            Some(league) => league,
            None => {return Err(Box::new(errors::InvalidInputError{description: "Could not find a league for picks"}) as BoxError)}
        };
